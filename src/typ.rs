...
impl Type {
    pub fn datatype(&self) -> String {
        ...
        match self {
            ...
            Type::Named(base, pars) if pars.len() == 0 && dts.contains(&base.as_str()) => {
                base.clone()
            }
            Type::And(ts) => {
                for t in ts.iter() {
    ...
    pub fn is_open(&self) -> bool {
        match self {
            ...
            Type::Named(tn, ts) => {
                tn.chars().all(char::is_uppercase) || ts.iter().any(|tt| tt.is_open())
            }
    ...
    pub fn and(&self, other: &Type) -> Type {
        match (self, other) {
            ...
            (l, r) => Type::And(vec![l.clone(), r.clone()]),
        }
    }
    ...
    pub fn normalize(&self) -> Type {
        match self {
            ...
            Type::Arrow(p, b) => Type::Arrow(Box::new(p.normalize()), Box::new(b.normalize())),
    ...
    pub fn remove(&self, x: &Type) -> Type {
        ...
        match self {
            ...
            Type::Named(tn, ts) => Type::Named(
                tn.clone(),
                ts.iter().map(|t| t.remove(x)).collect::<Vec<Type>>(),
            ),
            ...
            Type::Product(ts) => {
                Type::Product(ts.iter().map(|t| t.remove(x)).collect::<Vec<Type>>())
            }
    ...
    pub fn substitute(&self, subs: &HashMap<Type, Type>) -> Type {
        ...
        match self {
            ...
            Type::Arrow(p, b) => {
                Type::Arrow(Box::new(p.substitute(subs)), Box::new(b.substitute(subs)))
            }
            Type::Ratio(p, b) => {
                Type::Ratio(Box::new(p.substitute(subs)), Box::new(b.substitute(subs)))
            }
            Type::Named(tn, ts) => Type::Named(
                tn.clone(),
                ts.iter().map(|t| t.substitute(subs)).collect::<Vec<Type>>(),
            ),
            Type::And(ts) => {
                Type::And(ts.iter().map(|t| t.substitute(subs)).collect::<Vec<Type>>())
            }
            Type::Tuple(ts) => {
                Type::Tuple(ts.iter().map(|t| t.substitute(subs)).collect::<Vec<Type>>())
            }
            Type::HTuple(bt, ct) => Type::HTuple(Box::new(bt.substitute(subs)), ct.clone()),
            Type::Product(ts) => {
                Type::Product(ts.iter().map(|t| t.substitute(subs)).collect::<Vec<Type>>())
            }
            Type::Constant(cv) => Type::Constant(cv.clone()),
        }
    }
    ...
    pub fn arrow_implies(tlc: &TLC, lt: &mut Type, rt: &mut Type, inarrow: InArrow) -> Type {
        ...
        *rt = rt.normalize();
        lt.__implication_unifier(&rt, &mut subs, inarrow)
            .normalize()
    }
    ...
    fn __implication_unifier(
        &self,
        other: &Type,
        subs: &mut Vec<(Type, Type)>,
        inarrow: InArrow,
    ) -> Type {
        ...
        let tt = match (self, other) {
            ...
            (Type::Named(lv, lps), Type::Named(rv, rps)) if lv == rv && lps.len() == rps.len() => {
                let mut tps = Vec::new();
            ...
            (Type::Arrow(pl, bl), Type::Arrow(pr, br)) => {
                ...
                let bt = if **bl == Type::Any {
                    (**br).clone()
                } else {
                    bl.__implication_unifier(br, subs, InArrow::Rhs)
                };
                if bt.is_bottom() {
                    return bt.clone();
                }
            ...
            (Type::Constant(lv), Type::Constant(rv)) if lv == rv => Type::Constant(lv.clone()),
            _ => Type::And(vec![]),
        };
        tt
    }
    pub fn most_general_unifier(&self, other: &Type) -> Type {
        ...
        match (self, other) {
            ...
            (Type::Named(lv, _lps), Type::Named(rv, _rps))
                if lv.chars().all(char::is_uppercase) && lv == rv =>
            {
                self.clone()
            }
            ...
            (lt, Type::Ratio(pr, br)) => {
                //assert Nil divisor on rhs
                match **br {
                    Type::Tuple(ref bs) if bs.len() == 0 => lt.most_general_unifier(pr),
                    _ => Type::And(vec![]),
                }
            }
            (Type::Ratio(pl, bl), rt) => {
                //assert Nil divisor on rhs
                match **bl {
                    Type::Tuple(ref bs) if bs.len() == 0 => pl.most_general_unifier(rt),
                    _ => Type::And(vec![]),
                }
            }
