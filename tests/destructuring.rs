...
fn check_iflet_literal() {
   ...
   tlc.check(Some(l1), "if let 1 = 1 { 1 } else { 2 } @reduce : [1];").unwrap();
   tlc.check(Some(l1), "if let 1 = 2 { 1 } else { 2 } @reduce : [2];").unwrap();
   tlc.check(Some(l1), "if let 1 = 1 { 1 } else { 2 } @reduce : [3];").unwrap_err();
}
